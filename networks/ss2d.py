...
##########################################################
# csms6s.py  (synced from VMamba; supports oflex/core/mamba backends)
##########################################################
    ...
    def forward_corev2(
        ...
        **kwargs,
        ...
        if _scan_mode == -1:
            ...
            # Extract horizontal/vertical direction features for NematicRecomposition
            if return_directions and _scan_mode == 0:
                ys_flat = ys.view(B, K, D, -1)
                f_h = ys_flat[:, 0] + ys_flat[:, 2].flip(dims=[-1])
                f_h = f_h.view(B, D, H, W)
                f_v = ys_flat[:, 1] + ys_flat[:, 3].flip(dims=[-1])
                f_v = f_v.view(B, D, W, H).transpose(dim0=2, dim1=3).contiguous().view(B, D, H, W)
            else:
                f_h = f_v = None
        ...
        if return_directions:
            return y.to(x.dtype), f_h, f_v
        return y.to(x.dtype)

    def forwardv2(self, x: torch.Tensor, return_directions=False, **kwargs):
        x = self.in_proj(x)
        ...
        if self.with_dconv:
            x = self.conv2d(x) # (b, d, h, w)
        x = self.act(x)
        y = self.forward_core(x, return_directions=return_directions)
        if return_directions:
            y, f_h, f_v = y
        y = self.out_act(y)
        if not self.disable_z:
