...
class Mamba3(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.ssm_dt_scale = float(ssm_dt_scale)
        self.ssm_a_scale = float(ssm_a_scale)
        self.ssm_trap_scale = float(ssm_trap_scale)
        self.ssm_angle_scale = float(ssm_angle_scale)
        for name, value in {
            'ssm_dt_scale': self.ssm_dt_scale,
            'ssm_a_scale': self.ssm_a_scale,
            'ssm_trap_scale': self.ssm_trap_scale,
            'ssm_angle_scale': self.ssm_angle_scale,
        }.items():
            if value <= 0:
    ...
    def _apply_state_space_experiment(self, A, DT, trap, angles):
        A = A * self.ssm_a_scale
        DT = DT * self.ssm_dt_scale
        trap = trap * self.ssm_trap_scale
        angles = angles * self.ssm_angle_scale
        ADT = A * DT
        return A, DT, ADT, trap, angles
    ...
    def forward(self, u, seq_idx=None, cu_seqlens=None, inference_params=None):
        ...
        _A = rearrange(_A, "b l n -> b n l")
        _A, DT, ADT, trap, angles = self._apply_state_space_experiment(_A, DT, trap, angles)
    ...
    def _preprocess(self, A_proj, dd_dt, B, C, x, z, trap_proj, angle_proj):
        ...
        _A, DT, _ADT, trap, angles = self._apply_state_space_experiment(_A, DT, trap, angles)
