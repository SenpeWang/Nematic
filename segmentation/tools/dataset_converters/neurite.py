...
"""Convert Neurite dataset to mmsegmentation format.
...
Label values: 0 (background), 255 (neurite) -> normalized to 0/1.
"""
...
def generate_list(image_dir, list_file):
    """Generate image list file for MMSeg."""
    with open(list_file, 'w') as f:
        for img_name in sorted(scandir(image_dir, suffix='.png')):
...
def parse_args():
    parser = argparse.ArgumentParser(
        description='Convert Neurite dataset to mmsegmentation format')
    parser.add_argument('dataset_path', help='Neurite dataset path')
    parser.add_argument('-o', '--out-dir', help='output path')
    parser.add_argument(
        '--no-norm',
        action='store_true',
        help='skip label normalization if already 0/1')
